...
pub use super::{Coord, Size};

pub type CoordIter = super::CoordIterRowMajor;
pub type EdgeCoordIter = super::EdgeIter;
...
#[derive(Serialize, Deserialize, Reflect, Debug, Clone, Hash, PartialOrd, Ord, PartialEq, Eq)]
pub struct Grid<T> {
    cells: Vec<T>,
    size: Size,
}
...
impl<T> Grid<T> {
    ...
    pub fn try_new_iterator<I>(size: Size, iterator: I) -> Result<Self, IteratorLengthDifferentFromSize>
    ...
    where F: FnMut(Coord, U) -> T {
        let size = grid.size;
        let cells =
            size.coord_iter_row_major().zip(grid.cells.into_iter()).map(|(coord, u)| f(coord, u)).collect();
        Self { cells, size }
    }
    ...
    where F: FnMut(Coord, &U) -> T {
        let cells = grid.coord_iter().zip(grid.iter()).map(|(coord, cell)| f(coord, cell)).collect();
        Self {
            cells,
            size: grid.size,
        }
...
impl<T> Grid<T> {
    ...
    pub fn coord_iter(&self) -> CoordIter { self.size.coord_iter_row_major() }
    ...
    pub fn get_mut(&mut self, coord: Coord) -> Option<&mut T> {
        self.index_of_coord(coord).map(move |index| &mut self.cells[index])
    }

    pub fn get_tiled(&self, coord: Coord) -> &T { &self.cells[self.index_of_normalized_coord(coord)] }
    ...
    pub fn get_index_checked(&self, index: usize) -> &T { self.cells.index(index) }

    pub fn get_index_checked_mut(&mut self, index: usize) -> &mut T { self.cells.index_mut(index) }

    pub fn get_checked(&self, coord: Coord) -> &T { self.cells.index(self.index_of_coord_checked(coord)) }
    ...
    pub fn enumerate(&self) -> GridEnumerate<T> { self.coord_iter().zip(self.iter()) }

    pub fn enumerate_mut(&mut self) -> GridEnumerateMut<T> { self.coord_iter().zip(self.iter_mut()) }

    pub fn into_enumerate(self) -> GridIntoEnumerate<T> { self.coord_iter().zip(self.cells.into_iter()) }

    pub fn rows(&self) -> GridRows<T> { self.cells.chunks(self.size.width() as usize) }

    pub fn rows_mut(&mut self) -> GridRowsMut<T> { self.cells.chunks_mut(self.size.width() as usize) }
    ...
    pub fn map<U, F: FnMut(T) -> U>(self, f: F) -> Grid<U> { Grid::new_grid_map(self, f) }

    pub fn map_with_coord<U, F: FnMut(Coord, T) -> U>(self, f: F) -> Grid<U> {
        Grid::new_grid_map_with_coord(self, f)
    }

    pub fn map_ref<U, F: FnMut(&T) -> U>(&self, f: F) -> Grid<U> { Grid::new_grid_map_ref(self, f) }

    pub fn map_ref_with_coord<U, F: FnMut(Coord, &T) -> U>(&self, f: F) -> Grid<U> {
        Grid::new_grid_map_ref_with_coord(self, f)
    }

    pub fn is_on_edge(&self, coord: Coord) -> bool { self.size.is_on_edge(coord) }

    pub fn edge_coord_iter(&self) -> EdgeCoordIter { self.size.edge_iter() }
    ...
    pub fn edge_enumerate(&self) -> GridEdgeEnumerate<T> { self.edge_coord_iter().zip(self.edge_iter()) }
...
impl<'a, T> Iterator for GridEdgeIter<'a, T> {
    ...
    fn next<'b>(&'b mut self) -> Option<Self::Item> {
        self.edge_coord_iter.next().map(|coord| self.grid.get_checked(coord))
    }
}
...
mod tests {
    ...
    fn coord_grid(size: Size) -> Grid<Coord> { Grid::new_fn(size, |coord| coord) }
    ...
    fn enumerate() {
        ...
        grid.enumerate().for_each(|(coord, cell)| assert_eq!(coord, *cell));
        grid.enumerate_mut().for_each(|(coord, cell)| *cell = coord * 3);
        grid.enumerate().for_each(|(coord, cell)| assert_eq!(coord * 3, *cell));
    }
