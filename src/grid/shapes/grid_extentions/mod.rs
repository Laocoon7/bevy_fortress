...
pub trait ShapeGridExtensionsCopy<T: Copy> {
...
pub trait ShapeGridExtensionsClone<T: Clone> {
    /// Copy a shape off of the grid
    fn get_from_shape(&self, shape: impl Shape, unused_value: T) -> Self;
    /// Copy a shape onto the grid
    fn set_from_shape(&mut self, shape: impl Shape, value: T);
...
impl<T: Copy> ShapeGridExtensionsCopy<T> for Grid<T> {
    fn get_from_shape(&self, shape: impl Shape, unused_value: T) -> Self {
...
impl<T: Clone> ShapeGridExtensionsClone<T> for Grid<T> {
    fn get_from_shape(&self, shape: impl Shape, unused_value: T) -> Self {
        let rectangle = shape.aabb();
        let mut grid = Grid::new_clone(rectangle.size, unused_value);

        shape.for_each(|coord| {
            let grid_position =
                Coord::new(coord.x - rectangle.left(), coord.y - rectangle.bottom());
            if let Some(grid_value) = self.get(coord) {
                *grid.get_checked_mut(grid_position) = grid_value.clone();
            }
        });
