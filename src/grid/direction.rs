...
use std::{
    iter, mem,
    mem::MaybeUninit,
    ops::{BitAnd, BitAndAssign, BitOr, BitOrAssign, Index, IndexMut, Range},
    slice,
};
...
#[derive(Serialize, Deserialize, Reflect, Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(u8)]
pub enum Direction {
    North = 0,
    NorthEast,
...
impl From<CardinalDirection> for Direction {
    fn from(c: CardinalDirection) -> Self { c.direction() }
}

impl From<OrdinalDirection> for Direction {
    fn from(o: OrdinalDirection) -> Self { o.direction() }
}
...
impl DirectionBitmap {
    ...
    pub const fn empty() -> Self { NO_DIRECTIONS_BITMAP }

    pub const fn all() -> Self { ALL_DIRECTIONS_BITMAP }

    pub const fn all_cardinal() -> Self { ALL_CARDINAL_DIRECTIONS_BITMAP }
    ...
    pub const fn is_full(self) -> bool { self.raw == ALL_DIRECTIONS_BITMAP_RAW }

    pub const fn and(self, rhs: Self) -> Self { Self::new(self.raw & rhs.raw) }
...
macro_rules! make_direction_table {
    (
        ...
        pub type $enumerate_mut_type<'a, T> = iter::Zip<$direction_iter, DirectionTableIterMut<'a, T>>;
        impl<T> $table_type<T> {
    ...
    fn iteration() {
        {
            ...
            assert_eq!(CardinalDirections.into_iter().collect::<Vec<_>>(), vec![
                North, East, South, West
            ])
        }
        {
            use OrdinalDirection::*;
            assert_eq!(OrdinalDirections.into_iter().collect::<Vec<_>>(), vec![
                NorthEast, SouthEast, SouthWest, NorthWest
            ])
        }
        {
            use crate::grid::Direction::*;
            assert_eq!(Directions.into_iter().collect::<Vec<_>>(), vec![
